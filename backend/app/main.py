...
from fastapi import FastAPI, Depends, WebSocket, WebSocketDisconnect, HTTPException, status, Query, Request
...
# Load allowed origins from env or default
...
# Custom Rate Limiter dict
RATE_LIMIT_WINDOWS = defaultdict(list)

def check_rate_limit(client_ip: str, limit: int, window: int) -> bool:
    current_time = time.time()
    RATE_LIMIT_WINDOWS[client_ip] = [t for t in RATE_LIMIT_WINDOWS[client_ip] if current_time - t < window]

    # Occasional garbage collection of expired IP addresses (e.g. 1 in 100 requests)
    import random
    if random.random() < 0.01:
        dead_keys = [k for k, v in RATE_LIMIT_WINDOWS.items() if not v or (current_time - v[-1] >= window)]
        for k in dead_keys:
            del RATE_LIMIT_WINDOWS[k]

    if len(RATE_LIMIT_WINDOWS[client_ip]) >= limit:
        return False
    RATE_LIMIT_WINDOWS[client_ip].append(current_time)
    return True
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # Keep connection alive, limit message size (5KB max)
            data = await websocket.receive_text()
            if len(data) > 5120:
                await websocket.send_text("ERROR: Message size limit exceeded.")
                await websocket.close(code=status.WS_1009_MESSAGE_TOO_BIG)
                manager.disconnect(websocket)
                return
            await websocket.send_text(f"ACK: {data[:100]}")
...
async def report_incident(request: Request, payload: schemas.IncidentCreate, db: Session = Depends(get_db)):
    client_host = request.client.host if request.client else "unknown"
    if not check_rate_limit(client_host, limit=10, window=60):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many incidents reported. Please slow down.")
    # 1. Save incident
    inc = Incident(
        type=payload.type,
...
def ai_copilot_endpoint(request: Request, payload: schemas.CopilotQueryRequest, db: Session = Depends(get_db)):
    client_host = request.client.host if request.client else "unknown"
    if not check_rate_limit(client_host, limit=12, window=60):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many queries. Please wait a minute.")
